...
func (p *ProductRepository) Create(ctx context.Context, product *models.Product) (int, error) {
	stmt, err := p.db.PrepareContext(ctx, "INSERT INTO products (title, description, rating, category_id, brand_id, price, discount, tags, weight) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id")
	if err != nil {
...
func (p *ProductRepository) GetByID(ctx context.Context, id int) (*models.Product, error) {
	stmt, err := p.db.PrepareContext(ctx, "SELECT * FROM products WHERE id = $1")
	if err != nil {
	...
	if err = stmt.QueryRowContext(ctx, id).Scan(&pr.ID, &pr.Title, &pr.Description, &pr.Rating, &pr.CategoryID, &pr.BrandID, &pr.Price, &pr.Discount, pq.Array(&pr.Tags), &pr.Weight); err != nil {
...
func (p *ProductRepository) Update(ctx context.Context, pr *models.Product) error {
	stmt, err := p.db.PrepareContext(ctx, "UPDATE products SET title = $2, description = $3, rating = $4, category_id = $5, brand_id = $6, price = $7, discount = $8, tags = $9, weight = $10 WHERE id = $1")
	if err != nil {
		return fmt.Errorf("failed to prepare statement: %w", err.Error())
	}
	defer stmt.Close()
	if _, err = stmt.ExecContext(ctx, pr.ID, pr.Title, pr.Description, pr.Rating, pr.CategoryID, pr.BrandID, pr.Price, pr.Discount, pq.Array(pr.Tags), pr.Weight); err != nil {
...
func (p *ProductRepository) Delete(ctx context.Context, id int) error {
	stmt, err := p.db.PrepareContext(ctx, "DELETE FROM products WHERE id = $1")
	if err != nil {
		return fmt.Errorf("failed to prepare statement: %w", err.Error())
	}
	defer stmt.Close()
...
func (p *ProductRepository) GetAll(ctx context.Context) ([]models.Product, error) {
	stmt, err := p.db.PrepareContext(ctx, "SELECT * FROM products LIMIT 10")
	if err != nil {
